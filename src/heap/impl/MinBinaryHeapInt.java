...
public class MinBinaryHeapInt {
    ...
    private void heapifyUp() {
        ...
        int currIndex = size - 1; // current element index
        int newlyInsertedElement = heap[currIndex];

        while (currIndex > 0 && newlyInsertedElement < heap[getParentIndex(currIndex)]) {
            heap[currIndex] = heap[getParentIndex(currIndex)];
            currIndex = getParentIndex(currIndex);
        }
        heap[currIndex] = newlyInsertedElement;
    ...
    */
     ...
     * You only need to check floor(length(size)/2) last elements.
     * The nodes before that have children, so they never will be the max.
     * That means checking n/2 elements, so you still have O(n) complexity.
     **/
    public int findMax() {
